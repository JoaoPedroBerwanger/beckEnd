  ...
  <div class='wrap'>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if (isset($_POST['salvar'])) {
        ...
        if ($idPost > 0) {
          $sql = "UPDATE produto_grupo SET descricao = ?, idnAtivo = ? WHERE id = ?";
          ...
          $sql = "INSERT INTO produto_grupo (descricao, idnAtivo) VALUES (?, ?)";
          $stmt = $conn->prepare($sql);
    ...
    if ($modo === 'excluir' && $id > 0) {
      $conn->query("DELETE FROM produto_grupo WHERE id = $id");
    ...
    <?php if ($modo === 'novo' || ($modo === 'editar' && $id > 0)): ?>
      <?php if ($modo === 'editar') {
        $q = $conn->query("SELECT * FROM produto_grupo WHERE id = $id");
        $row = $q->fetch_assoc();
      } ?>
      <div class='form-card'>
        <form method='POST'>
